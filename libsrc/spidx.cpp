...
#define SPI_TIMEOUT     (1000L)  //1000 ms
 ...
 *    the base I/O address of the external SPI interface of Vertex86 DX.
 *    
...
RBAPI(bool) spi_WriteFlush(unsigned char val) {
    if (spi_Write(val)  == false)  return false;
    if (spi_FIFOFlush() == false)  return false;
    return true;
}
...
RBAPI(unsigned) spi_ReadStrict(void) {
	...
	for (i=0; i<20; i++)
	{
		...
		if (spi_Read() == 0xffff) return 0xffff;
	}
